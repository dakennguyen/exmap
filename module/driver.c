...
struct exmap_ctx {
  ...
  /* Interfaces are memory mapped aread where the kernel can communicate to the
   * user
   */
  int max_interfaces;
...
static inline int exmap_account_mem(struct exmap_ctx *ctx,
  ...
  pr_info("page_limit: %ld/%ld (alloc: %lu)\n",
          atomic_long_read(&ctx->user->locked_vm), page_limit, nr_pages);
  ...
  do {
    ...
    if (new_pages > page_limit)
      return -ENOMEM;
  } while (atomic_long_cmpxchg(&ctx->user->locked_vm, cur_pages, new_pages) !=
           cur_pages);
...
static unsigned free_pages_move(struct free_pages *src, struct free_pages *dst,
                                unsigned max_pages) {
  ...
  /* remove <steal_count> elements from the victim and add them to the robber's
   * list */
...
static int steal_pages(struct exmap_ctx *ctx, struct exmap_interface *interface,
   ...
   */
  first_steal_id =
      choose_and_lock_steal_interface(ctx, interface, desired_count);
...
again: // steal_from->free_pages must be locked.
  ...
  if (this_steal_count > 0) {
    ...
    // this_steal_count, 		steal_from->free_pages.count, 		steal_count
    // +
    // this_steal_count, desired_count);

    steal_count +=
        free_pages_move(&steal_from->free_pages, dst, this_steal_count);
    steal_from->count.s++;
...
static int exmap_alloc_pages(struct exmap_ctx *ctx,
                             struct exmap_interface *interface,
                             struct free_pages *pages, int min_pages) {
  int steal_count;
#ifdef USE_SYSTEM_ALLOC
#ifdef USE_BULK_SYSTEM_ALLOC
  unsigned long ret =
      alloc_pages_bulk_list(GFP_NOIO | __GFP_ZERO, min_pages, &pages->list);
...
void exmap_free_page(struct exmap_ctx *ctx, struct exmap_interface *interface,
                     struct page *page) {
/* #if 0 */
#ifdef USE_SYSTEM_ALLOC
...
void exmap_free_pages(struct exmap_ctx *ctx, struct exmap_interface *interface,
...
static void vm_close(struct vm_area_struct *vma) {
  ...
  for (idx = 0; idx < ctx->max_interfaces; idx++) {
    ...
    for (it_idx = 0; it_idx < interface->shadow_cnt; it_idx++) {
      addr = vma->vm_start +
             ((interface->shadow_pages[it_idx].page_id) << PAGE_SHIFT);
      exmap_remove_shadow_pages(vma, addr,
                                interface->shadow_pages[it_idx].count);
    }
  }
...
static vm_fault_t vm_fault(struct vm_fault *vmf) {
  ...
  rc = exmap_alloc_pages(ctx, interface, &free_pages, 1);
  if (rc < 0)
    return VM_FAULT_SIGSEGV;

  rc = exmap_insert_pages(vma, (uintptr_t)vmf->address, 1, &free_pages, NULL,
                          NULL);
  if (rc < 0)
    return VM_FAULT_SIGSEGV;
 ...
 */
static void vm_open(struct vm_area_struct *vma) { pr_info("vm_open\n"); }
...
static void exmap_vma_cleanup(struct exmap_ctx *ctx, unsigned long start,
                              unsigned long end) {
...
static int
exmap_notifier_invalidate_range_start(struct mmu_notifier *mn,
...
static int exmap_mmap(struct file *file, struct vm_area_struct *vma) {
  ...
  if (offset == EXMAP_OFF_EXMAP) {
    ...
    vm_open(vma);
  } else if (offset >= EXMAP_OFF_INTERFACE_BASE &&
             offset <= EXMAP_OFF_INTERFACE_MAX) {
...
int exmap_alloc(struct exmap_ctx *ctx, struct exmap_action_params *params) {
  ...
  rc = exmap_alloc_pages(ctx, interface, &free_pages, iov_len);
  if (rc) {
  ...
  for (idx = 0; idx < iov_len; idx++) {
    ...
    // pr_info("alloc[%d]: off=%llu, len=%d", iface, (uint64_t) vec.page, (int)
    // vec.len);
    ...
    rc = exmap_insert_pages(vma, uaddr, vec.len, &free_pages, NULL, &alloc_ctx);
    if (rc < 0)
      failed++;
    ...
    exmap_debug("alloc: %llu+%d => rc=%d, used=%d", (uint64_t)vec.page,
                (int)vec.len, (int)ret.res, (int)ret.pages);
...
int exmap_remove_shadow(struct exmap_ctx *ctx,
  ...
  for (idx = 0; idx < interface->shadow_cnt; idx++) {
    addr =
        vma->vm_start + ((interface->shadow_pages[idx].page_id) << PAGE_SHIFT);
    exmap_remove_shadow_pages(vma, addr, interface->shadow_pages[idx].count);
    exmap_debug("remove shadow[%d]: page=%llu, adr=%llu, len=%d, idx=%d", iface,
                (uint64_t)interface->shadow_pages[idx].page_id,
...
int exmap_shadow(struct exmap_ctx *ctx, struct exmap_action_params *params) {
  int iface = params->interface;
  struct exmap_interface *interface = &(ctx->interfaces[iface]);
  struct vm_area_struct *vma = ctx->exmap_vma;
  unsigned int iov_len = params->iov_len;
  ...
  for (idx = 0; idx < iov_len; idx++) {
    ...
    exmap_debug("shadow[%d]: off=%llu, len=%d, freed: %lu, rc: %ld", iface,
                (uint64_t)vec.page, (int)vec.len, rc);
  }
...
int exmap_free(struct exmap_ctx *ctx, struct exmap_action_params *params) {
  int iface = params->interface;
  struct exmap_interface *interface = &(ctx->interfaces[iface]);
  struct vm_area_struct *vma = ctx->exmap_vma;
  unsigned int iov_len = params->iov_len;
  ...
  for (idx = 0; idx < iov_len; idx++) {
    ...
    exmap_debug("free[%d]: off=%llu, len=%d, freed: %lu", iface,
...
typedef int (*exmap_action_fptr)(struct exmap_ctx *,
...
static long exmap_ioctl(struct file *file, unsigned int cmd,
                        unsigned long arg) {
  ...
  case EXMAP_IOCTL_SETUP:
    ...
    gfp_flags = GFP_KERNEL_ACCOUNT | __GFP_ZERO | __GFP_NOWARN | __GFP_COMP |
                __GFP_NORETRY;
    ctx->interfaces = __vmalloc_array(
        setup.max_interfaces, sizeof(struct exmap_interface), gfp_flags);
    if (!ctx->interfaces) {
...
ssize_t exmap_alloc_iter(struct exmap_ctx *ctx,
                         struct exmap_interface *interface,
  ...
  rc = exmap_alloc_pages(ctx, interface, &free_pages, total_nr_pages);
  if (rc < 0)
    return rc;
  ...
  while (iov_iter_count(iter)) {
    ...
    //  PAGE_SHIFT, iov_iter_count(iter));
    ...
    rc = exmap_insert_pages(ctx->exmap_vma, (uintptr_t)addr,
                            (size >> PAGE_SHIFT), &free_pages, NULL, NULL);
    if (rc < 0)
      return rc;
    rc_all += rc;
...
static const struct file_operations fops = {.mmap = exmap_mmap,
                                            .open = open,
...
static int dev_uevent_perms(const struct device *dev,
...
module_init(exmap_init_module) module_exit(exmap_cleanup_module)
    MODULE_LICENSE("GPL");
